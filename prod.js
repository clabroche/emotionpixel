...
(async _ => {
  ...
  await bash('docker', ["build", "-t", "emotionpixel", "."])
  await bash('docker', ["save", "-o", "emotionpixel.tar", "emotionpixel"])
...
async function createFolderIfNeeded() {
  ...
  if(!folders.includes('emotionpixel')) await ssh.mkdir('/app/emotionpixel')
}
...
async function createComposeIfNeeded() {
  ...
  const files = await ls('/app/emotionpixel')
  if(!files.includes('docker-compose.yml')) await ssh.putFile('./docker-compose.yml', '/app/emotionpixel/docker-compose.yml')
}

async function pushDockerImage() {
  const res = await ssh.execCommand('stat -c "%s" /app/emotionpixel/emotionpixel.tar')
  const remoteSize = +res.stdout
  const res1 = await bashExec('stat -c "%s" ./emotionpixel.tar')
  const localSize = +res1
  if(remoteSize !== localSize) {
    console.log('Push docker image...')
    await PromiseB.map([
      progressFile(),
      ssh.putFile('./emotionpixel.tar', '/app/emotionpixel/emotionpixel.tar')
    ], _ => {})
  } else {
...
async function loadDockerImageOnServer() {
  ...
  await ssh.execCommand('docker load -i /app/emotionpixel/emotionpixel.tar')
}

async function relaunchDockerCompose () {
  console.log('Relaunch docker...')
  await ssh.execCommand('cd /app/emotionpixel && docker-compose up -d')
}
